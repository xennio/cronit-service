...
class RestTaskActorTest extends TestKit(ActorSystem("CronitTestActorSystem"))
  ...
  it should "call http end point with given parameters when body is not present" in {
    ...
    expectedRequest.entity shouldEqual HttpEntities.EMPTY
  }

  it should "call http end point with given parameters without headers when body is not present" in {
    val mockHttp = mock[HttpExt]
    ...
    expectedRequest.method shouldEqual HttpMethods.POST
    expectedRequest.uri shouldEqual Uri(restJobModel.url)
    expectedRequest.headers.size shouldEqual 0
  }
